...
document.addEventListener('mousedown', function (event) {
    ...
    if (event.target.closest('#out')) {
        ...
        // Proceed with the selection box logic if not clicking on a cell
        ui.selectedCells = [];
        ...
        if (event.target.id === 'resize-handle') {
            ...
            startX = event.clientX - diagramRect.left;
            startY = event.clientY - diagramRect.top;
...
document.addEventListener('mousemove', function (event) {
    if (dragStart && !ui.isResizing && isSelecting) {
        const diagramRect = diagramElement.getBoundingClientRect();
        currentX = event.clientX - diagramRect.left;
        currentY = event.clientY - diagramRect.top;
...
document.addEventListener('mouseup', function (event) {
    if (dragStart && !ui.isResizing && isSelecting) {
        ...
        const boxRect = getSelectionBBox(startX, startY, currentX, currentY);
        diagramElement.removeChild(selectionBox);

        istar.graph.getElements().forEach(function (cell) {
            ...
            // Check if the element's bounding box intersects with the selection box
            if (
                ...
                ui.selectedCells.push(cell);
                // Apply the same dashed-line selection box as single selection
                var selectionBox = document.createElement("div");
                selectionBox.classList.add("cell-selection");
                selectionBox.style.position = "absolute";
                selectionBox.style.border = "3px dashed rgb(105, 105, 105)";
                selectionBox.style.background = "rgba(0, 0, 0, 0)";
                selectionBox.style.left = `${cellRect.left - 6}px`;
                selectionBox.style.top = `${cellRect.top - 6}px`;
                selectionBox.style.width = `${bbox.width + 12.5}px`;
...
function getSelectionBBox(startX, startY, currentX, currentY) {
    return {
...
function updateSidePanel() {
    ...
    document.getElementById('generate-report').onclick = function () {
        console.log(ui.selectedCells.length);
